...
def create_schedule(stud, courses, course_dict):
    ...
    while len(all_courses) > 0 and year <= stud.years_to_grad:
        ...
        semester_full = schedule_semester(courses_left, course_dict, graph_nodes_dict, semester_object, program)
...
def remove_scheduled_courses(all_courses, semester_object):
    for sections in semester_object.sections:
        try:
...
def schedule_semester(all_courses, course_dict, graph_nodes_dict, semester, program):
    ...
    while len(semester.sections) < max_courses_per_semester and super_safe_double_check and len(all_courses) > 0:
        ...
        if not course_node.taken:
            all_courses.append(course_name)
    if len(semester.sections) < semester.max_courses or super_safe_double_check is False:
